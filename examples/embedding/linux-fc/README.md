...
`mp_embed_exec_string_function(name, arg)` function.  It allows a C host to
compile and execute a Python source snippet, retrieve a named callable defined
in that snippet, invoke it with a single string argument, and receive the
return value back as a C string.
